...
def calulate_confint_of_effect_size(
    mean_1, mean_2, sd_1, sd_2, size_1, size_2, effect_size_method
):
    ...
    sigma_d = np.sqrt(
        ((size_1 + size_2) / (size_1 * size_2)) + (d**2 / (2 * (size_1 + size_2)))
    )
...
def calculate_confidence_interval(mean_1, mean_2, sd_1, sd_2, size_1, size_2):
    ...
    print_interval = [
        "[lower: {}] [mean: {}] [upper: {}]".format(
            round(lower_limit, 5), round(mean_diff, 5), round(upper_limit, 5)
        ),
        [lower_limit, mean_diff, upper_limit],
    ]
...
if __name__ == "__main__":
    ...
    Hedges_g = calculate_Hedges_g(
        sampleMean_1, sampleMean_2, sampleSD_1, sampleSD_2, sampleSize_1, sampleSize_2
    )
    CI95 = calculate_confidence_interval(
        sampleMean_1, sampleMean_2, sampleSD_1, sampleSD_2, sampleSize_1, sampleSize_2
    )
    ...
    print(
        " \n #----------\n # Results\n #----------\n \n Cohen's d = {} \n Glass's delta = {} \n d = {} \n 95% CI = {} \n {}".format(
            Cohens_D, Glass_delta, Hedges_g, CI95, note
        )
    )
