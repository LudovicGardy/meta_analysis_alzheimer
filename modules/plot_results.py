...
def plot_meta_analysis(nb_plots: int, 
    ...
    for i in range(nb_plots):
        ...
        ax.plot([row["CI95inf"], row["CI95sup"]], [i, i], color="dimgrey", linewidth=width_dict[row["Color"]], zorder=1)
        ax.scatter(row["d"], [i], color="black", s=sizes_dict[row["Color"]], marker=markers_dict[row["Color"]], zorder=2)
        ax.text(-3.8, i, row["Author"], fontsize=14)
        N_subjects = int(round(row["MCI_size"], 0)) + int(round(row["Control_size"], 0))
        ax.text(4, i, f"{N_subjects} ({int(round(row['MCI_size'], 0))}; {int(round(row['Control_size'], 0))})", fontsize=14)
    ...
    ax.text(-3.8, -5, "Random effects model", fontweight='bold', bbox={'facecolor': 'lightgray', 'alpha': 0.5, 'pad': 5}, fontsize=14)
    ...
    print(f"Nb studies = {nb_studies} | Tau squared = {round(T_squared, 3)} | {p_val_text} | {IC95text} | Fail Safe N = {fail_safe_N}")
...
if __name__ == '__main__':
    ...
    fail_safe_N = 999
    plot_meta_analysis(nb_plots, table, weight_of_studies, effect_size, nb_studies, T_squared, p_val_text, IC95text, fail_safe_N)
