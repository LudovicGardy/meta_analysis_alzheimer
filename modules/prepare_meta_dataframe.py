...
def prepare_meta_dataframe(input_data, studies_with_multiple_measures):
    ...
    for i in range(len(input_data["MCI_Mean"])):
        if not (np.isnan(input_data["MCI_Mean"][i]) or np.isnan(input_data["Control_Mean"][i]) or 
                np.isnan(input_data["MCI_SD"][i]) or np.isnan(input_data["Control_SD"][i]) or 
                np.isnan(input_data["MCI_size"][i]) or np.isnan(input_data["Control_size"][i])):
            ...
            meta_dict["Color"].append("orange" if input_data["Authors"][i] not in studies_with_multiple_measures else "black")
    ...
    for study in studies_with_multiple_measures:
        ...
        if indices:
            ...
            meta_dict["Control_size"].append(meta_frame.loc[indices, "Control_size"].mean())
            meta_dict["MMSE_score"].append(meta_frame.loc[indices, "MMSE_score"].mean())
            meta_dict["Color"].append("orange")

    meta_frame = pd.DataFrame(meta_dict)
    meta_frame = meta_frame.sort_values(by='Weight', ascending=True).reset_index(drop=True)
